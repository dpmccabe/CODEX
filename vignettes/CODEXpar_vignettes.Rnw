...
%\VignetteIndexEntry{Using CODEXpar}
\begin{document}
\SweaveOpts{concordance=TRUE}
\title{CODEXpar vignette}
\author{Yuchao Jiang
    \\
    \href{mailto:yuchaoj@wharton.upenn.edu}{yuchaoj@wharton.upenn.edu}}
\maketitle
This is a demo for using the \verb@CODEXpar@ package in R. \verb@CODEXpar@ is a 
normalization and copy number variation calling procedure for whole exome DNA 
sequencing data. \verb@CODEXpar@ relies on the availability of multiple samples 
processed using the same sequencing pipeline for normalization, and does not 
require matched controls. The normalization model in \verb@CODEXpar@ includes 
terms that specifically remove biases due to GC content, exon capture and 
amplification efficiency, and latent systemic artifacts. \verb@CODEXpar@ also 
...
analysed for illustration purposes. R packages are available at Bioconductor for \href{http://www.bioconductor.org/packages/devel/bioc/html/CODEXpar.html}{CODEXpar} and the toy dataset \href{http://www.bioconductor.org/packages/devel/data/experiment/html/WES.1KG.WUGSC.html}{WES.1KG.WUGSC}.

\section*{1. Installation and online forum}
Installation information is available \href{https://sites.google.com/a/cornell.edu/yuchaojiang/home/research/codexpar}{here}. The online Q\&A forum is available \href{https://groups.google.com/d/forum/codexpar_wes_cnv}{here}. If you've any questions regarding the software, please don't hesitate emailing us at \href{mailto:codexpar_wes_cnv@googlegroups.com}{codexpar\_wes\_cnv@googlegroups.com} and/or \\
\href{mailto:yuchaoj@wharton.upenn.edu}{yuchaoj@wharton.upenn.edu}.

\section*{2. CODEXpar workflow:}

\subsection*{2.1 Get directories of .bam files, read in exon target positions 
from .bed files, and get sample names.}
The direct input of \verb@CODEXpar@ include: \verb@bamdir@, which is a vector indicating the
...
chromosome. \verb@CODEXpar@ processes the entire genome chromosome by chromosome; make sure 
...
bambedObj <- getbambed(bamdir = bamdir, bedFile = bedFile, 
                       sampname = sampname, projectname = "CODEXpar_demo", chr)
bamdir <- bambedObj$bamdir; sampname <- bambedObj$sampname
ref <- bambedObj$ref; projectname <- bambedObj$projectname; chr <- bambedObj$chr
@

\subsection*{2.2 Get raw read depth from the .bam files. Read lengths across all 
...
\subsection*{2.3 Compute GC content and mappability for each exon target.}
<<gcmapp1>>=
gc <- getgc(chr, ref)
mapp <- getmapp(chr, ref)
@

\subsection*{2.4 Take a sample-wise and exon-wise quality control procedure on 
...
\subsection*{2.5 Fit Poisson latent factor model for normalization of the read 
depth data.}
<<normObj1>>=
...
@
If the WES is designed under case-control setting, CODEXpar estimates the exon-wise
...
regression. \verb@normal_index@ specifies the indices of normal samples:
<<normObj2, eval=FALSE>>=
normObj <- normalize(Y_qc, gc_qc, K = 1:9, normal_index=seq(1,45,2))
Yhat <- normObj$Yhat; AIC <- normObj$AIC; BIC <- normObj$BIC
RSS <- normObj$RSS; K <- normObj$K
@
\subsection*{2.6 Determine the number of latent factors. AIC, BIC, and deviance 
reduction plots are generated in a .pdf file.}
CODEXpar reports all three statistical metrics (AIC, BIC, percent of Variance 
...
normalization step cannot be recovered, CODEXpar opts for a more conservative 
...
\subsection*{2.7 Fit the Poisson log-likelihood ratio based segmentation 
procedure to determine CNV regions across all samples.}
For germline CNV detection, \verb@CODEXpar@ uses the "integer" mode; for CNV 
detection involving large recurrent chromosomal aberrations in mixture 
populations (e.g. somatic CNV detection in cancer),  \verb@CODEXpar@ opts to use
the "fraction" mode.
\\
...
write.table(finalcall, file = paste(projectname, '_', chr, '_', optK,
            '_CODEXpar_frac.txt', sep=''), sep='\t', quote=FALSE, row.names=FALSE)
...
\section*{3. Citation}
CODEXpar: a normalization and copy number variation detection method for whole exome sequencing
